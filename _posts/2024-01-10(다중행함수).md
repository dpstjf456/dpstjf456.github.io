...
|순번|함수명|설명|사용방법|
|---|---|---|---|
|1|sum(option='distinct or all(default)) column|입력된 컬럼의 합계를 출력(distinct 입력 시 중복값 제외)| select sum(sal) from scott.emp;
|1|count(option='distinct or all(default)) column|입력된 컬럼의 개수를 출력(distinct 입력 시 중복값 제외)| select count(sal) from scott.emp;
|1|max(option='distinct or all(default)) column|입력된 컬럼 중 최댓값을 출력(distinct 입력 시 중복값 제외)|select max(sal) from scott.emp;
|1|min(option='distinct or all(default)) column|입력된 컬럼 중 최솟값을 출력 출력(distinct 입력 시 중복값 제외)| select min(sal) from scott.emp;
|1|avg(option='distinct or all(default)) column|입력된 컬럼 중 평균값을 출력 출력(distinct 입력 시 중복값 제외)| select avg(sal) from scott.emp;
