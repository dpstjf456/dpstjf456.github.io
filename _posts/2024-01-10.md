...
|2|count(columm)|columm로 입력된 다중행 데이터의 수를 계산| select sum(sal) from scott.emp;|
|3|max(columm)|columm로 입력된 다중행 데이터 중 최댓값을 계산| select sum(sal) from scott.emp;|
|4|min(columm)|columm로 입력된 다중행 데이터 중 최솟값을 계산| select sum(sal) from scott.emp;|
|5|avg(columm)|columm로 입력된 다중행 데이터 중 평균값을 계산| select sum(sal) from scott.emp;|
